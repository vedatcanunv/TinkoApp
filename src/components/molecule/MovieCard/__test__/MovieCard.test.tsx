...
describe('MovieCard Component', () => {
  ...
  it('renders with different sizes', () => {
    const {toJSON: small} = render(<MovieCard movie={mockMovie} size="small" />);
    const {toJSON: medium} = render(<MovieCard movie={mockMovie} size="medium" />);
    const {toJSON: large} = render(<MovieCard movie={mockMovie} size="large" />);
  ...
  it('handles movie without poster', () => {
    ...
    const {toJSON} = render(
      <MovieCard movie={movieWithoutPoster} placeholderText="Film Posteri Yok" />
    );
  ...
  it('renders with custom styles', () => {
    const {toJSON} = render(<MovieCard movie={mockMovie} style={{margin: 10, borderRadius: 8}} />);
