...
describe("MediaCard Component", () => {
  ...
  it("calls onPress when card is pressed", () => {
    ...
    const { getByText } = render(
      <MediaCard media={mockMedia} onPress={onPressMock} />
    );
  ...
  it("renders with different sizes", () => {
    const { toJSON: small } = render(
      <MediaCard media={mockMedia} size="small" />
    );
    const { toJSON: medium } = render(
      <MediaCard media={mockMedia} size="medium" />
    );
    const { toJSON: large } = render(
      <MediaCard media={mockMedia} size="large" />
    );
  ...
  it("renders with custom style", () => {
    const { toJSON } = render(
      <MediaCard media={mockMedia} style={{ margin: 10, borderRadius: 15 }} />
    );
  ...
  it("respects maxGenreTags prop", () => {
    ...
    const { toJSON: max2 } = render(
      <MediaCard media={mockMedia} maxGenreTags={2} />
    );
    const { toJSON: max1 } = render(
      <MediaCard media={mockMedia} maxGenreTags={1} />
    );
  ...
  it("renders without type label when showType is false", () => {
    ...
    const { toJSON: withoutType } = render(
      <MediaCard media={mockMedia} showType={false} />
    );
