...
describe('DetailInfoRow Component', () => {
  it('renders correctly', () => {
    const {toJSON} = render(<DetailInfoRow label="Yönetmen" value="Christopher Nolan" />);
    expect(toJSON()).toMatchSnapshot();
  });

  it('displays label and value correctly', () => {
    const {getByText} = render(<DetailInfoRow label="Tür" value="Bilim Kurgu" />);
  ...
  it('renders with long label and value', () => {
    ...
    expect(getByText('Çok çok uzun bir değer metni ve detaylandırılmış bilgi')).toBeTruthy();
    expect(toJSON()).toMatchSnapshot('long content');
  });

  it('renders with empty value', () => {
    const {toJSON, getByText, queryByText} = render(<DetailInfoRow label="Açıklama" value="" />);
