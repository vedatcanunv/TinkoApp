...
export const ProfileMediaList: React.FC<ProfileMediaListProps> = ({
  ...
  return (
    <View style={[styles.container, style]}>
      <MediaListHeader title={title} activeFilter={activeFilter} onFilterChange={setActiveFilter} />
      <FlatList
        data={mediaData}
        renderItem={({item}) => (
          <MediaCard media={item} onPress={() => onMediaPress(item)} style={styles.mediaCard} />
        )}
        numColumns={2}
        ...
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            <Text size="m" color="light">
              {activeFilter === 'watched' ? 'Henüz izlenen içerik yok' : 'İzleme listesi boş'}
            </Text>
          </View>
        )}
      />
