...
describe('HomeScreen Component', () => {
  ...
  it('renders loading state initially', async () => {
    const {getByText} = render(<HomeScreen onMoviePress={jest.fn()} onAddPress={jest.fn()} />);
  ...
  it('calls onMoviePress when media card is pressed', async () => {
    ...
    const {findByText} = await act(async () =>
      render(<HomeScreen onMoviePress={onMoviePressMock} onAddPress={jest.fn()} />)
    );
  ...
  it('calls onAddPress when add button is pressed', async () => {
    ...
    const {getByTestId} = await act(async () =>
      render(<HomeScreen onMoviePress={jest.fn()} onAddPress={onAddPressMock} />)
    );
